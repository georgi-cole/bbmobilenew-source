// MODULE: houseguests.js
// Complete houseguest data source with basic info and dramatic stories
// This is the single source of truth for AI houseguest information
// Used in intro hub modal and in-game avatar taps

;(function (global) {
  'use strict'

  // Full houseguest data with basic info and advanced story
  // NOTE: To add/update guest data, modify entries below. Each guest must have:
  // - id: stable lowercase identifier (e.g., 'kian')
  // - name: display name (e.g., 'Kian')
  // - motto, funFact, allies, enemies: optional fields for profile popup
  const HOUSEGUESTS = [
    {
      id: 'finn',
      name: 'Finn',
  ...
  /**
   * Get all houseguests data
   * @returns {Array} Array of houseguest objects with full information
   */
  function getAllHouseguests() {
    return HOUSEGUESTS.slice() // Return a copy
  }

  /**
   * Get houseguest by name
   * @param {string} name - Houseguest name
   * @returns {object|null} Houseguest object or null if not found
   */
  function getHouseguest(name) {
    return HOUSEGUESTS.find((h) => h.name === name) || null
  }

  /**
   * Get houseguest by stable ID
  ...
  function getHouseguestById(id) {
    ...
    return HOUSEGUESTS.find((h) => h.id === normalized) || null
  }

  /**
   * Get houseguests by player list (filter to only AI players in current game)
   * @param {Array} players - Array of player objects from game.players
   * @returns {Array} Array of houseguest objects matching the player names
   */
  function getHouseguestsByPlayers(players) {
    if (!Array.isArray(players)) return []
    const names = players
      .filter((p) => p && !p.human) // Only AI players
      .map((p) => p.name)
    return HOUSEGUESTS.filter((h) => names.includes(h.name))
  }

  // Expose to global scope
  global.Houseguests = {
    getAll: getAllHouseguests,
    get: getHouseguest,
    getById: getHouseguestById,
    getByPlayers: getHouseguestsByPlayers,
    data: HOUSEGUESTS,
  }

  console.info('[houseguests] Data source loaded with', HOUSEGUESTS.length, 'houseguests')
})(window)
