...
export const selectConfessionalAlertCount = (state: RootState): number => {
  ...
  if (
    state.game?.bellaWill?.privateBriefingPending &&
    state.game.bellaWill.privateBriefingInvited
  ) {
    count += 1
  }
  ...
  // The rewarded vote reveal is session-scoped rather than Redux state. Its
  // Confessional badge becomes available only after Bella's public Last Will
  // card and any human-heir briefing have had their uninterrupted moment.
  const voteBreakdownUnlock = loadEvictionVoteBreakdownUnlock()
  if (
    !state.game.bellaWill?.publicAnnouncementPending &&
    !state.game.bellaWill?.privateBriefingPending &&
