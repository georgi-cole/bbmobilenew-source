...
/** Publish the only public detail of an inherited Bella power: it has been spent. */
function pushBellaWillCompletion(state: GameState, phase: Phase) {
  if (!state.bellaWill || state.bellaWill.completionAnnouncementPending) return
  state.bellaWill.completionAnnouncementPending = true
  pushEvent(state, "Bella's Last Will is now complete.", 'game', {
    major: 'bellas_will_complete',
...
function getBellaWillPublicModeExceptionNotice(
  state: GameState,
  lastPlaceId: string | null | undefined
): string | null {
  if (!lastPlaceId || !isBellaHeirImmune(state, lastPlaceId)) return null

  const heirName = state.players.find((player) => player.id === lastPlaceId)?.name ?? "Bella's heir"
  return `Bella's Last Will protects ${heirName}, so the automatic third nomination and today's public save will not occur.`
}
...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    commitNominees(state, action: PayloadAction<string[]>) {
      ...
      const bellaWillPublicModeNotice = canUsePublicNomineeRule
        ? getBellaWillPublicModeExceptionNotice(state, state.lastHohCompFinisherId)
        : null
      ...
      const autoNomineeClause = autoNomineePlayer
        ...
        : null
      const nominationEventText = autoNomineeClause
        ? `${nameList} have been nominated for elimination. ${autoNomineeClause}. 🎯`
        : `${nameList} have been nominated for elimination by ${hohName}. 🎯`
      const eventText = bellaWillPublicModeNotice
        ? `${nominationEventText} ${bellaWillPublicModeNotice}`
        : nominationEventText
      ...
      pushEvent(
        ...
        eventText,
        'game',
        bellaWillPublicModeNotice
          ? {
              major: 'bellas_will',
              broadcastLevel: 'major',
              broadcastDelivery: 'next',
              forceOnTv: true,
              announcementTitle: "Bella's Last Will",
              announcementSubtitle: bellaWillPublicModeNotice,
            }
          : undefined
      )
    },
    ...
    finalizePendingEviction(state, action: PayloadAction<string>) {
      ...
      if (evicteeId === BELLA_ID && state.bellaWill?.active && !state.bellaWill.inherited) {
        ...
        // An inheritance can only start after Bella has actually left the
        // house. Debug controls may preselect the heir and reward, but they
        // must never make the reward usable before this boundary.
        ...
        if (state.bellaWill.inherited && heir && reward) {
          pushEvent(
            state,
            `Bella's final message names ${heir.name} as her heir. The inheritance remains private until it is used.`,
            'game',
            {
              major: 'bellas_will',
              broadcastLevel: 'critical',
              broadcastPriority: 'critical',
              // The exit animation owns eviction results. Show this next, at
              // the week-end handoff, so it cannot be rebuilt out of the queue
              // before the Faux TV gets a render.
              phase: 'week_end',
            }
          )
        }
    ...
    forcePlayerStatus(
      ...
      // The QA control must exercise the same post-exit handoff as a real
      // eviction. Otherwise forcing Bella to leave makes the selected heir
      // and reward look as though they were never saved.
      if (
        playerId === BELLA_ID &&
        (status === 'evicted' || status === 'jury') &&
        state.bellaWill?.active &&
        !state.bellaWill.inherited
      ) {
        const currentHeir = state.players.find(
          (candidate) => candidate.id === state.bellaWill?.heirId
        )
        const currentHeirIsValid =
          currentHeir != null &&
          currentHeir.id !== BELLA_ID &&
          currentHeir.status !== 'evicted' &&
          currentHeir.status !== 'jury'
        if (!currentHeirIsValid) state.bellaWill.heirId = chooseBellaHeir(state)
        activateBellaInheritance(state)
        const heir = state.players.find((candidate) => candidate.id === state.bellaWill?.heirId)
        if (state.bellaWill.inherited && heir) {
          pushEvent(
            state,
            `Bella's final message names ${heir.name} as her heir. The inheritance remains private until it is used.`,
            'game',
            { major: 'bellas_will', broadcastLevel: 'critical', phase: state.phase }
          )
        }
      }
    ...
    debugSetBellaWillReward(state, action: PayloadAction<BellaWillReward>) {
      ...
      state.bellaWill.privateBriefingPending = false
      state.bellaWill.privateBriefingInvited = false
      state.bellaWill.publicAnnouncementPending = false
      state.bellaWill.completionAnnouncementPending = false
    ...
    debugActivateBellaInheritance(state) {
      ...
      // Never grant the reward merely because it was selected in Debug. The
      // production exit path is the single source of truth for activation.
      if (bella && bella.status !== 'evicted' && bella.status !== 'jury') return
      if (!state.bellaWill.heirId) state.bellaWill.heirId = chooseBellaHeir(state)
      activateBellaInheritance(state)
      state.bellaWill.debugForced = true
    },
    acknowledgeBellaWillBriefing(state) {
      if (state.bellaWill?.privateBriefingPending) {
        state.bellaWill.privateBriefingPending = false
      }
    },
    acknowledgeBellaWillPublicAnnouncement(state) {
      const will = state.bellaWill
      if (!will?.publicAnnouncementPending) return
      will.publicAnnouncementPending = false
      if (will.privateBriefingPending) will.privateBriefingInvited = true
    },
    acknowledgeBellaWillCompletionAnnouncement(state) {
      if (state.bellaWill?.completionAnnouncementPending) {
        state.bellaWill.completionAnnouncementPending = false
      }
    },
    /**
      ...
      const hydrated: GameState = {
        ...
        bellaWill: action.payload.bellaWill
          ? {
              ...
              // Saves made before the dedicated post-exit handoff did not
              // record whether this first card was seen. Replay it once so a
              // human heir cannot be left without the private briefing route.
              publicAnnouncementPending:
                action.payload.bellaWill.publicAnnouncementPending ??
                action.payload.bellaWill.inherited ??
                false,
              completionAnnouncementPending:
                action.payload.bellaWill.completionAnnouncementPending ?? false,
              privateBriefingPending: action.payload.bellaWill.privateBriefingPending ?? false,
              privateBriefingInvited: action.payload.bellaWill.privateBriefingInvited ?? false,
    ...
    advance(state) {
      ...
      switch (nextPhase) {
        ...
        case 'week_start': {
          ...
          if (expireBellaWillImmunity(state)) {
            pushBellaWillCompletion(state, 'week_start')
          }
          state.currentWeekNominationRecord = null
          state.lohId = null
        ...
        case 'nomination_results': {
          ...
          const canUsePlannedBlock =
            ...
            plannedNominees.every((candidate) => !isBellaHeirImmune(state, candidate.id))
          ...
          const nominationEventText = `${nameList} have been nominated for elimination. 🎯`
          pushEvent(
            state,
            bellaWillPublicModeNotice
              ? `${nominationEventText} ${bellaWillPublicModeNotice}`
          ...
          if (
            ...
            if (heir && !heir.isUser && eligibleVoterIds.has(heirId) && !existingBonusBallot) {
              ...
              if (eligibleTargets.length > 0 && voteMap[heirId]) {
                ...
                pushBellaWillCompletion(state, 'eviction_results')
              }
            }
          }
        ...
        case 'eviction_results': {
          ...
          if (
            ...
            if (rawVotesAgainstHeir > 0 && !competingDeduction) {
              ...
              pushBellaWillCompletion(state, 'week_end')
            }
          }
...
export const {
  ...
  acknowledgeBellaWillBriefing,
  acknowledgeBellaWillPublicAnnouncement,
  acknowledgeBellaWillCompletionAnnouncement,
