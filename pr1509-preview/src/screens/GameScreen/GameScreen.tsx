...
import { BELLA_ID, BELLA_MINIGAME_AVATAR } from '../../features/twists/bellasWill'
...
export default function GameScreen() {
  ...
  const confessionalTvAnnouncement =
    confessionalPrompt.triggered && confessionalPrompt.visible
      ...
      : null
  // Bella's exit deliberately owns the next Faux-TV beat. This stays outside
  // the broadcast queue because the exit animation transitions phase first.
  const bellaWillTvAnnouncement: Announcement | null = game.bellaWill?.publicAnnouncementPending
    ? {
        key: 'bellas_will',
        title: "Bella's Last Will",
        subtitle: (() => {
          const heir = game.players.find((player) => player.id === game.bellaWill?.heirId)
          return heir
            ? `Bella's final message names ${heir.name} as her heir. The inheritance remains private until it is used.`
            : "Bella's final message has named an heir. The inheritance remains private until it is used."
        })(),
        isLive: true,
        autoDismissMs: null,
      }
    : game.bellaWill?.completionAnnouncementPending
      ? {
          key: 'bellas_will_complete',
          title: "Bella's Last Will Complete",
          subtitle: "Bella's private legacy has been used. Her final wish is complete.",
          isLive: true,
          // This is the final beat of an already-resolved power. It must not
          // require an available Play action to clear, because the game can be
          // waiting on its next phase while this acknowledgement is on screen.
          autoDismissMs: 4200,
        }
      : null
  const priorityTvAnnouncement = bellaWillTvAnnouncement ?? confessionalTvAnnouncement
  const dismissPriorityTvAnnouncement = useCallback(() => {
    if (game.bellaWill?.publicAnnouncementPending) {
      dispatch(acknowledgeBellaWillPublicAnnouncement())
      return
    }
    if (game.bellaWill?.completionAnnouncementPending) {
      dispatch(acknowledgeBellaWillCompletionAnnouncement())
      return
    }
    dismissConfessionalTvPrompt()
  }, [dispatch, dismissConfessionalTvPrompt, game.bellaWill])
      ...
      >
        {showPublicSaveReveal && publicSaveWinnerId ? (
          <TvZone
            ...
            priorityAnnouncement={priorityTvAnnouncement}
            onPriorityAnnouncementDismiss={dismissPriorityTvAnnouncement}
          ...
          <MinigameHost
            ...
            participants={pendingChallenge.participants.map((id): MinigameParticipant => {
              ...
              return {
                ...
                // Bella's competition card deliberately uses her grey-lux studio
                // portrait; the main roster uses her black-and-gold house portrait.
                avatar: player?.id === BELLA_ID ? BELLA_MINIGAME_AVATAR : player?.avatar,
