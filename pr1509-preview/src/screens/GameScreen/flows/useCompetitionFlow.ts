...
export function useCompetitionFlow({
  ...
  // The outgoing human LOH is correctly excluded from the next LOH contest.
  // There is no playable challenge in that case, so advance the state machine
  // to its deterministic result instead of leaving the Game screen on an empty
  // competition phase.
  useEffect(() => {
    if (
      game.phase === 'loh_comp' &&
  ...
  }, [game.phase, humanIsOutgoingHoh, pendingChallenge, pendingWinnerCeremony, dispatch])
