...
export function useEvictionFlow({
  ...
  const capturePostEvictionVoteBreakdown = useCallback(
    (evictee: Player) => {
      if (
  ...
  const queueVoteBreakdownConfessionalOffer = useCallback(
    (snapshot: VoteBreakdownSnapshot) => {
      ...
      // This event causes the Confessional badge to update without taking a
      // Faux-TV slot from a high-priority eviction or twist announcement.
      dispatch(
        addTvEvent({
  ...
  const handleVoteResultsDone = useCallback(() => {
    ...
    if (evictee && game.pendingEviction) {
      ...
      capturePostEvictionVoteBreakdown(evictee)
    ...
    // No clear evictee (tie or edge case): keep the reveal optional and route
    // it to the Confessional instead of blocking the results with an ad modal.
    if (canOfferVoteBreakdown && !hasActiveVoteBreakdownUnlock()) {
      queueVoteBreakdownConfessionalOffer({
        gameId: game.gameId,
        week: game.week,
        phase: game.phase,
        votes: { ...(game.votes ?? {}) },
    ...
    canOfferVoteBreakdown,
    capturePostEvictionVoteBreakdown,
    game,
    hasActiveVoteBreakdownUnlock,
    proceedAfterVoteResults,
    queueVoteBreakdownConfessionalOffer,
  ...
  const handleTiebreakerRequired = useCallback(
    (tiedIds: string[]) => {
      ...
      if (!humanIsHoH) {
        ...
        capturePostEvictionVoteBreakdown(aiTiebreakContext.evictee)
  ...
  // local state machine. The optional vote reveal is then queued in the Confessional.
  const handleEvictionSplashDone = useCallback(() => {
    ...
    if (postEvictionVoteSnapshotRef.current && !hasQueuedPartnerEviction) {
      queueVoteBreakdownConfessionalOffer(postEvictionVoteSnapshotRef.current)
      postEvictionVoteSnapshotRef.current = null
    }
  }, [dispatch, game, queueVoteBreakdownConfessionalOffer, setFinal4Stage])
