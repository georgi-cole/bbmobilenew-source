...
export default function FloatingActionBar({
  ...
  const secretMissionRewardPendingKey =
    !isSurvivorMode && game.secretMission?.status === 'rewardPending'
      ...
      : null
  const bellaWillBriefingKey =
    !isSurvivorMode &&
    game.bellaWill?.privateBriefingPending &&
    game.bellaWill.privateBriefingInvited &&
    game.bellaWill.heirId === humanPlayer?.id
      ? `${game.week}:${game.bellaWill.reward ?? 'unknown'}`
      : null
  const previousSecretMissionRewardPendingKeyRef = useRef<string | null>(null)
  const previousBellaWillBriefingKeyRef = useRef<string | null>(null)
  ...
  // Bella's private inheritance briefing uses the same explicit Confessional
  // spotlight as a completed Secret Mission. The public TV card does not name
  // the heir or reward, so this is the player's clear personal invitation.
  useEffect(() => {
    if (bellaWillBriefingKey === null) {
      previousBellaWillBriefingKeyRef.current = null
      return
    }
    if (previousBellaWillBriefingKeyRef.current === bellaWillBriefingKey) return
    previousBellaWillBriefingKeyRef.current = bellaWillBriefingKey
    const showTimer = window.setTimeout(() => setShowSecretMissionRewardSpotlight(true), 0)
    return () => window.clearTimeout(showTimer)
  }, [bellaWillBriefingKey])
