...
function reducer(_state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'FETCH_OK':
      return { status: 'ok', week: action.week }
    case 'FETCH_ERR':
...
export default function DiaryWeekView({ seasonId, weekNumber }: Props) {
  ...
  useEffect(() => {
    ...
    getDiaryWeek(seasonId, weekNumber)
      .then((week) => {
        if (!cancelled) dispatch({ type: 'FETCH_OK', week })
      })
      .catch((err: unknown) => {
        if (!cancelled)
  ...
  if (state.status === 'error') {
    return (
      <p className="dw-view__error" role="alert">
        ⚠️ {state.message}
      </p>
    )
  }
            ...
            <div className="dw-view__tally">
              ...
              {tally.map(([name, count]) => (
                <div key={name} className="dw-view__tally-row">
                  ...
                  <span className="dw-view__tally-count">
        ...
        <section className="dw-view__section">
          ...
          <ol className="dw-view__timeline">
            {week.socialEvents.map((ev, i) => (
              <li key={i} className="dw-view__timeline-item">
                {ev}
              </li>
            ))}
          </ol>
      ...
      {(week.misc.length > 0 || week.notes) && (
        <section className="dw-view__section">
          ...
          {week.misc.length > 0 && (
            <ul className="dw-view__list">
              {week.misc.map((m, i) => (
                <li key={i} className="dw-view__list-item">
                  {m}
                </li>
              ))}
            </ul>
