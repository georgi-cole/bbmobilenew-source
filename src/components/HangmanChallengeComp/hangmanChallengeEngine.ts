...
import configuredWords from './verdictBoardWords.json'
import configuredBoosts from './verdictBoardBoosts.json'
...
const LEGACY_WORD_BANK: WordEntry[] = [
  {
    text: 'shock',
    category: 'event',
    difficulty: 1,
...
const CONFIGURED_WORD_BANK: WordEntry[] = configuredWords.map((text, index) => {
  const category = ['arcade', 'strategy', 'timing', 'competition', 'puzzle'][index % 5]
  const difficulty = ((index % 5) + 1) as WordDifficulty
  return {
    text,
    category,
    difficulty,
    cluePool: [`A ${category} term used in games`, `Think about ${category} and play`],
  }
})

const WORD_BANK: WordEntry[] = [...LEGACY_WORD_BANK, ...CONFIGURED_WORD_BANK]

const LEGACY_MYSTERY_BOX_POOL: MysteryBoxDefinition[] = [
  {
    id: 'reveal_one',
...
void LEGACY_MYSTERY_BOX_POOL

export const MYSTERY_BOX_POOL: MysteryBoxDefinition[] = configuredBoosts.map((boost) => ({
  ...boost,
  category: boost.category as MysteryBoxCategory,
}))
...
export function calculateRoundScore({
  ...
  if (!solved) {
    const failureAdjustments = mysteryAdjustments.reduce((sum, item) => sum + item.value, 0)
    const bonuses = bonusTokenPoints > 0 ? [{ label: 'Token bonus', value: bonusTokenPoints }] : []
    return {
      baseScore: 0,
      errorPenalty: 0,
      timePenalty: 0,
      mysteryAdjustments,
      bonuses,
      // Failed rounds do not earn a base score, but Mystery Box score changes
      // must still have a real consequence. This keeps risks and banked tokens
      // truthful instead of silently discarding them on a failed board.
      finalRoundScore: failureAdjustments + bonusTokenPoints,
    }
  }
  ...
  if (perfectEligible && errors === 0 && elapsedSeconds <= 25) {
